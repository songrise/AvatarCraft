...
from utils import utils, ray_utils, debug_utils
from utils.constant import SMPL_SCALE
...
class NeuManReader():
    ...
    @classmethod
    def read_scene(cls, scene_dir, tgt_size=None, normalize=False, bkg_range_scale=1.1, human_range_scale=1.1, mask_dir='segmentations', smpl_type='romp', keypoints_dir='keypoints', densepose_dir='densepose', rest_pose = None):
        ...
        scene.scale = scale
        smpls, world_verts, static_verts, Ts = cls.read_smpls(scene_dir, scene.captures, scale=scale, smpl_type=smpl_type, rest_pose = rest_pose)
    ...
    @classmethod
    def read_smpls(cls, scene_dir, caps, scale=1, smpl_type='romp',rest_pose =None, **kwargs):
        ...
        for cap in caps:
            ...
            _, T_t2pose,_ = body_model.verts_transformations(
                return_tensor=False,
                poses=temp_smpl['pose'][None],
                betas=zero_shape,
                concat_joints=True
            )
            _, T_t2da, _ = body_model.verts_transformations(
                return_tensor=False,
                poses=da_smpl,
                betas=zero_shape,
                concat_joints=True
    ...
    def read_smpls_avatar(cls, scene_dir, scale=1, smpl_type='romp',rest_pose = None, poses = None):
        ...
        shape_path = "/workspace/songrise/NeuMan/debug/betas_fat.pkl"
        with open(shape_path, 'rb') as f:
            shapes = np.load(f).astype(np.float32) 
        shape_path = "/workspace/songrise/NeuMan/debug/betas_skinny.pkl"
        with open(shape_path, 'rb') as f:
            shapes2 = np.load(f).astype(np.float32)*1.4
        ###########interpolation
        zero_shape = np.zeros((1, 10)) 
        zero_pose = np.zeros((1, 24, 3)) 
        target_shape = shapes
        target_pose = poses
        # interpolate betas
        n_shapes, n_pose = 10,4
        target_shapes = np.linspace(shapes2, target_shape, n_shapes)
        target_shapes = np.concatenate([target_shapes, target_shapes[::-1]])
        target_poses = np.linspace(zero_pose, target_pose, n_pose)
        # #grid
        # all_betas = []
        for i in range(poses.shape[0]):
            # assert 0 <= frame_id < len(caps)
            temp_smpl = extract_smpl_at_frame(raw_smpl, 0)
            # zero_pose = np.zeros_like(temp_smpl['pose'][None])
            da_smpl = np.zeros_like(temp_smpl['pose'][None])
            da_smpl = da_smpl.reshape(-1, 3)
            da_smpl[1] = np.array([0, 0, 1.0])
            da_smpl[2] = np.array([0, 0, -1.0])
            ...
            _, T_t2pose, __ = body_model.verts_transformations(
                return_tensor = False,
                # poses = poses[i],
                # poses = target_poses[target[i%16][1]],
                poses = poses[i,...][None],
                # poses = t,
                betas = zero_shape,
                concat_joints = True
            ...
            # the warp caused by beta blended shape
            delta_v = delta_v.squeeze()
            zero_v = np.zeros((24,3))
            delta_v = np.concatenate((delta_v,zero_v), axis=0)
            T_t2rest = ray_utils.batch_add_translation(T_t2rest, delta_v)
            # 90 degree rotation around x axis
            T_scale = np.eye(4) / SMPL_SCALE
            T_rest2pose = T_t2pose @ np.linalg.inv(T_t2rest)
            # T_rest2pose = np.linalg.inv(T_t2rest)
            ...
            # utils.save_mesh(rest_pose_verts, body_model.faces,"./neuman_T.obj")
            # rest_pose_verts, rest_pose_joints = np.squeeze(v_shaped,axis = 0), np.reshape(rest_pose, (-1, 3))
            temp_world_verts = np.einsum('BNi, Bi->BN', T_rest2scene, ray_utils.to_homogeneous(np.concatenate([rest_pose_verts, rest_pose_joints], axis=0)))[:, :3].astype(np.float32)
            temp_world_verts, temp_world_joints = temp_world_verts[:6890, :], temp_world_verts[6890:, :]
            # utils.save_mesh(temp_world_verts, body_model.faces, "./debug_avatar_SMPL.obj")
            temp_smpl['joints_3d'] = temp_world_joints
            temp_smpl['static_joints_3d'] = rest_pose_joints
            smpls.append(temp_smpl)
            Ts.append(T_rest2scene_nerf)
            static_verts.append(rest_pose_verts)
            world_verts.append(temp_world_verts)
        # debug_utils.dump_tensor(all_betas, "./debug_avatar_betas.pkl")
        return smpls, world_verts, static_verts, Ts
