...
def main_canonical_360(opt):
    ...
    for pose_name, render_poses in zip(['body', 'head'], [body_poses, head_poses]):
        ...
        if opt.log_extra:
            ...
            with open(intrisic_path, 'wb') as f:
                pickle.dump(can_cap.intrinsic_matrix, f) # share the same intrinsic matrix for all poses
    ...
    # mesh_path = os.path.join('./demo', 'canonical_360', opt.exp_name, f'{opt.exp_name}.ply')
    # if opt.implicit_model == "instant_nsr":
...
if __name__ == "__main__":
    ...
    parser.add_argument('--implicit_model',default = "instant_nsr", choices = ["neus","nerf","instant_nsr"],type = str,help = "choose the implicit model")
    parser.add_argument('--log_extra', default=False, type=options.str2bool, help='whether to log extra info (depth, camera extrinsic Nx4x4, camera intrinsic 3x3)')
