...
def print_opt(opt):
    ...
    #todo dump to file
    if not os.path.exists(os.path.join('./style', f'canonical_360/{opt.exp_name}', opt.exp_name)):
        os.makedirs(os.path.join('./style', f'canonical_360/{opt.exp_name}', opt.exp_name))
    save_path = os.path.join('./style', f'canonical_360/{opt.exp_name}', opt.exp_name, f'{opt.exp_name}_args.txt')
    # sort by first non-space character
    content_list.sort(key=lambda x: x.lstrip().split()[0])
...
def set_trajectory_option(parser):
    ...
    trajectory_opt.add_argument('--trajectory_resolution', default=60, type=int)
